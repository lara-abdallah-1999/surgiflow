...
useEffect(() => {
  ...
  return <header aria-label="Selected patient context" data-compact={compact} data-expanded={expanded} className="patient-context-header shrink-0 border-b border-slate-200 bg-white">
    ...
    <div id="patient-context-details" className="patient-context-grid grid gap-1.5 p-2">
      <section className="flex min-w-0 items-center gap-2.5 rounded-xl border border-violet-100 bg-gradient-to-r from-violet-50/60 to-white px-3 py-2.5">
        ...
        <div className="min-w-0"> 
          ...
          <p className="mt-1 text-[9px] text-slate-500">{[record.mrn || surgery.patientId, record.gender || record.patientGender || record.sex, typeof record.age === "number" ? record.age + " y" : undefined].filter(Boolean).join(" · ")}</p>
          <p className="mt-1 text-[9px] font-semibold text-slate-600">{surgery.doctor || "Doctor not recorded"}</p>
          <span className="mt-1 inline-block rounded-md bg-violet-50 px-1.5 py-0.5 text-[8px] font-bold text-violet-700">{surgery.status}</span>
        </div>
      </section>
      <section className="grid grid-cols-2 gap-2 rounded-xl border border-slate-100 px-3 py-2 text-[9px]">
      ...
      <section className="min-w-0 rounded-xl border border-slate-100 px-3 py-2">
        <p className="flex items-center gap-1.5 text-[8px] font-bold uppercase tracking-wide text-slate-400"><Scissors size={11} className="text-violet-500" /> Procedures & surgical sites</p>
        <ol className="mt-1.5 max-h-24 space-y-1 overflow-y-auto">
          {procedures.map((procedure, index) => <li key={index} className="flex items-start gap-1.5 text-[9px]">
            <span className="rounded bg-violet-50 px-1 text-violet-600">{index + 1}</span><div className="min-w-0 break-words"><p className="font-semibold text-slate-700">{procedure.name}</p><p className="text-[8px] text-slate-500">Site: {procedure.site || "Not recorded"}</p></div>
          </li>)}
          {!procedures.length && <li className="text-[9px] text-slate-400">Not recorded</li>}
        </ol>
      </section>
      <section className="grid grid-cols-2 gap-2 rounded-xl border border-slate-100 px-3 py-2 text-[9px]">
        <Fact label="Cashier" value={surgery.paymentStatus} />
        <Fact label="Pre-Op" value={surgery.preOpStatus} />
    ...
    <div ref={toolsRef} className="empty:hidden" />
  </header>;
}

function Fact({ label, value, icon: Icon }: { label: string; value: string; icon?: typeof Clock3 }) {
  return <div className="min-w-0"><p className="flex items-center gap-1 text-[8px] font-semibold text-slate-400">{Icon && <Icon size={11} className="shrink-0 text-violet-500" />}{label}</p><p className="mt-0.5 break-words font-semibold text-slate-700">{value}</p></div>;
}
